...
- **beat_scheduler.py** — Scheduler custom de Beat (`--scheduler=beat_scheduler:SchedulerDB`) que carga el schedule al arrancar y relee la DB cada 60s. Crear/editar/desactivar tareas cuya funcion ya existe **no requiere reiniciar**. Codigo Python nuevo si requiere reiniciar el worker.
